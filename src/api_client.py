...
class FoxWarApiClient:
    def get_war_data():       #May be generalized in behavior later to allow GETs to Baker and Charlie
    ...
    def get_map_data():       #May be generalized in behavior later to allow GETs to Baker and Charlie
        r = requests.get(f"{BASE_URL}/maps/")
        r.raise_for_status()
        return r.json()
